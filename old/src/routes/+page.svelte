<script lang="ts">
	...
	let lists: Observable<Record<'edited' | 'idle', ContentList[]>>;
	...
	$: {
		if (mounted && data?.lists) {
			lists = liveQuery(async () => {
				...
				return {
					edited: data.lists.filter(({ key }) => editedLists.includes(key)),
					idle: data.lists.filter(({ key }) => !editedLists.includes(key))
				};
			});
		}
	}
