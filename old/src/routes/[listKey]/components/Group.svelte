<script lang="ts">
	...
	import type { ContentListItem } from '$lib/model/list';
	...
	export let items: (ContentListItem & ItemState)[];
	...
	<ExpansionPanel disabled={!items?.length}>
		<div slot="header" class="flex">
			...
			<div class="flex-1">
				...
				<div
					class="text-neutral-500 text-[10px] leading-[7px] dark:text-neutral-300 transition-colors"
				>
		...
		{#each items as item (item.key)}
			<div class="sm:pl-10" transition:slide|local>
				<SwipeAction
					on:right={() => dispatcher('right', { itemKey: item.key })}
					rightColor={swipeActionColor}
				>
					{#key item.key}
						<Checkbox
							class="checkbox"
							checked={item.state}
							on:change={(e) =>
								dispatcher('change', { checked: e.target.checked, itemKey: item.key })}
						>
							{item.label}
						</Checkbox>
					{/key}
