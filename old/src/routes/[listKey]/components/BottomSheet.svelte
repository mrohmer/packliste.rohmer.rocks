<script lang="ts">
	...
	import type { ContentList } from '$lib/model/list';
	...
	const onResetStateClick = () => db.items.where({ listKey: list.key }).modify({ state: false });
		...
		if (mounted && list) {
			...
			total = liveQuery(async () => {
				...
				const relevantItems = items.filter(
					({ key }) => !irrelevantItems.some(({ itemKey }) => itemKey === key)
				);

				return relevantItems.length;
			});
			irrelevant = liveQuery(() =>
				db.items
					.where({ listKey: list.key })
					.filter(({ irrelevant }) => irrelevant)
					.count()
	...
	>
		<div class="px-2.5 pb-1 flex items-center max-w-4xl mx-auto">
			...
			<div class="h-full flex-1 text-sm py-1">
				<div>
					<span class="text-primary font-semibold dark:font-bold transition-colors"
						>{$done ?? 0}</span
					>
