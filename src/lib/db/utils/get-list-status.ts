...
const getTotal = async (list: IList): Promise<number> => {
  ...
  return list
      ...
      .filter(({key}) => !irrelevantItems.some(({itemKey}) => itemKey === key))
      .length
    ?? 0;
}

export const getListStatus = async (list: IList): Promise<Record<'completed' | 'total', number>> => {
