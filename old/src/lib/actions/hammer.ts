const hammerAction = (...events: string[]) => async (node: HTMLElement) => {
  ...
  const handlers = events
    .map(event => [event, (ev: any) => node.dispatchEvent(new CustomEvent(event, {detail: ev}))]);
